...
export function getEntryBySlug(type: string, slug: string, fields: string[] = []) {
  const realSlug = slug.replace(/\.md$/, '')
  const fullPath = type === 'posts' ? join(postsDirectory, `${realSlug}.md`) : join(tilDirectory, `${realSlug}.md`)
